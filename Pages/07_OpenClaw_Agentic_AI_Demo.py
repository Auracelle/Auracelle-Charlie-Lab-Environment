...
"""
pages/07_OpenClaw_Agentic_AI_Demo.py — Bounded agentic experimentation surface.

Gated by: authentication.
Governance: E-AGPO-HT remains the governing law.
All outputs are draft-only until explicitly approved.
Data access: read-only scenario context from session_state.
"""
import hashlib
import io
...
auth_ok = any(
    bool(st.session_state.get(k, False))
    for k in ("authenticated", "logged_in", "is_authenticated")
)
if not auth_ok:
...
READ_ONLY_KEYS = [
    "selected_policy", "policy_selected", "policy_scenario",
    "country_options", "roles", "research_session_id",
    "simulation_results", "risk_scores", "stress_test_results", "session_log",
]

ACCEPTED_TYPES = ["pdf", "docx", "txt", "doc"]
MAX_UPLOAD_MB  = 10

# Full policy list — mirrors config/policies.yaml; used when session state is unavailable
...
def first_present(keys: list, default=None):
    for key in keys:
...
def resolve_policy_options() -> list[str]:
    handles = None
    ...
    if not options:
        for key in ("policy_options", "POLICY_OPTIONS"):
            ...
            if isinstance(value, list) and value:
                options = value
                break
    return options or DEFAULT_POLICY_OPTIONS


def get_read_only_context() -> dict:
    context = {
    ...
    context["policy_source"] = (
        "OpenClaw selector"
        if st.session_state.get("openclaw_selected_policy")
        else "Shared session state"
    )
    return context


def log_governance_event(event_type: str, detail: dict) -> None:
...
def _safe_preview(text: str, limit: int = 3000) -> str:
    ...
    return text[:limit] + "\n\n...[preview truncated]..." if len(text) > limit else text
...
def extract_text_from_upload(uploaded_file) -> tuple[str, str]:
    ...
    if ext == "pdf":
        ...
        return "\n".join((p.extract_text() or "") for p in reader.pages), ext
    if ext == "docx":
        doc = Document(io.BytesIO(raw))
        return "\n".join(p.text for p in doc.paragraphs), ext
    if ext == "doc":
        raise ValueError(
            "Legacy .doc upload detected. "
            "Please convert to .docx, .pdf, or .txt first."
        )
    raise ValueError(f"Unsupported file type: {ext or 'unknown'}")


def draft_output(task_type: str, context: dict, operator_note: str) -> str:
    ...
    evidence_line = (
        f"Evidence Intake: {', '.join(evidence_names[:5])}"
        if evidence_names else "Evidence Intake: No uploaded evidence in buffer"
    )

    if task_type == "Generate narrative response draft":
        body = [
            "Narrative Draft:",
            f"Under {GOVERNING_LAW}, this draft frames a bounded response to stakeholder objections "
            "without altering the official policy text.",
            f"The current scenario centres on: {policy_name}.",
            "Recommended posture: acknowledge institutional concerns, explain rationale, "
            "and offer reversible concessions only.",
            evidence_line,
        ]
    elif task_type == "Generate bounded amendment draft":
        body = [
            "Bounded Amendment Draft:",
            f"Proposed amendment language should remain subordinate to {GOVERNING_LAW} "
            "and must preserve the original policy intent.",
        ...
        body = [
            ...
            f"Challenge the policy from the perspective of institutional stress, public legitimacy, "
            f"and geopolitical exploitation risks linked to {policy_name}.",
        ...
        body = [
            "Round Summary Draft:",
            f"This draft summarises the current scenario around {policy_name}.",
...
with left:
    ...
    default_index = (
        policy_options.index(inherited_policy)
        if inherited_policy in policy_options else 0
    )
    selected_policy = st.selectbox(
        "Select policy (mirrors Simulation page)",
        policy_options,
        index=default_index,
        help="Mirrors the Simulation page policy universe for standalone OpenClaw use. "
             "Read-only context — not the canonical policy authority.",
    )
    ...
    operator_note = st.text_area(
        ...
        placeholder="Example: Draft a response to objections about enforceability "
                    "while preserving governance safeguards.",
        height=110,
    )
...
with uploader_col:
    ...
    st.caption(
        "Accepted types: PDF, DOCX, TXT. Legacy DOC flagged for conversion. "
        "Uploads are parsed into a read-only evidence buffer."
    )
    uploaded_file = st.file_uploader(
        "Upload evidence file",
        type=ACCEPTED_TYPES,
        accept_multiple_files=False,
        help="Evidence is held for read-only review on this page only.",
    )
    ...
    if uploaded_file is not None:
        if st.button("Ingest into Evidence Intake", use_container_width=True):
            try:
                ...
                log_governance_event("evidence_buffered", {
...
with evidence_col:
    ...
    if not evidence_items:
        ...
        st.text_area("Read-only evidence preview", value=selected_evidence["preview"],
                     height=280, disabled=True)
        st.caption(
            "Evidence Intake is read-only. Uploaded content may inform drafts but "
            "cannot directly alter official Charlie state."
        )
...
if st.button("Generate draft output", use_container_width=True):
    ...
    log_governance_event("draft_generated", {
        "task_type":       task_type,
        "nemo_isolation":  bool(nemo_enabled),
        "approval_required": bool(approval_required),
...
if not pending:
    ...
    st.text_area("Pending draft content", pending["content"], height=340,
                 key="openclaw_pending_text")
    ...
    with review_col1:
        if st.button("Approve draft", use_container_width=True):
            ...
            st.success(
                "Draft approved. It remains separate from the official session log "
                "until an explicit write is allowed."
            )
    with review_col2:
...
if not approved_outputs:
    ...
    st.text_area("Approved draft preview", latest["content"], height=220,
                 key="approved_preview")
    ...
    if approval_required and allow_official_write:
        ...
        st.caption(
            "Official log writes are disabled unless approval is required "
            "and explicit write permission is turned on above."
        )
